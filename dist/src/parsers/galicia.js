...
function parseBlock(lines) {
    ...
    if (totalIdx !== -1)
        fullText = fullText.substring(0, totalIdx);
    ...
    if (saldoPos !== -1)
        desc = desc.substring(0, saldoPos);
    if (validNumbers.length >= 2) {
        const montoStr = validNumbers[validNumbers.length - 2];
        const montoPos = desc.lastIndexOf(montoStr);
        if (montoPos !== -1)
            desc = desc.substring(0, montoPos);
    }
    desc = cleanDesc(desc);
    if (desc.length < 3)
        return null;
...
//# sourceMappingURL=galicia.js.map