...
export const MunicipalParser: IParser = {
  ...
  parse(rawText: string): RawMovement[] {
    ...
    while (i < lines.length) {
      ...
      const fecha = normalizeDate(fechaMatch[1]);
      ...
      while (j < lines.length) {
        ...
        if (!next) { j++; continue; }
        if (RE_FECHA.test(next) || shouldSkip(next)) break;
      ...
      while ((m = RE_AMOUNT_GLOBAL.exec(content)) !== null) amounts.push(m[0]);

      if (amounts.length === 0) { i = j; continue; }

      const saldo = parseAmount(amounts[amounts.length - 1]);
