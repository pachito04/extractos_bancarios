...
export const CredicoopParser: IParser = {
  ...
  parse(rawText: string): RawMovement[] {
    ...
    for (const line of lines) {
      if (line.includes('SALDO ANTERIOR')) {
        ...
        if (m) { saldoAnterior = parseAmount(m[1]); break; }
      }
    }

    const movements: RawMovement[] = [];
    let currentMovement: { fecha: string; saldo: number } | null = null;
    let currentDesc = '';
    ...
    for (const rawLine of lines) {
      ...
      if (fechaMatch) {
        // Flush previous
        if (currentMovement) {
          const { debito, credito } = deltaDC(currentMovement.saldo, saldoAnterior);
          movements.push({
            fecha: currentMovement.fecha,
            descripcion: cleanDesc(currentDesc),
            debito,
            credito,
        ...
        if (!saldoMatch) { currentMovement = null; continue; }

        const saldoActual = parseAmount(saldoMatch[1]);
        const fecha = normalizeDate(fechaMatch[0]);
