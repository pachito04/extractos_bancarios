 ...
 */
export function validateMovements(movements: NormalizedMovement[]): ValidatedMovement[] {
  const result: ValidatedMovement[] = [];
  ...
  for (let i = 0; i < movements.length; i++) {
    ...
    const issues: string[] = [];
    let status: 'valid' | 'doubtful' | 'invalid' = 'valid';
    let confidence = mov.confidence;

    const setDoubtful = (reason: string) => {
      if (status !== 'invalid') status = 'doubtful';
      issues.push(reason);
    };
    ...
    if (saldoAnterior !== null && i > 0) {
      const esperado = round2(saldoAnterior + credito - debito);
      const diff = Math.abs(round2(esperado - saldo));
      if (diff > 0.02) {
        if (diff > 1000) {
    ...
    if (saldoAnterior !== null) {
      const salto = Math.abs(saldo - saldoAnterior);
      if (salto > 100_000 && debito === 0 && credito === 0) {
